...
export default function DashboardSidebar({ className = '', onToggle }: DashboardSidebarProps) {
  ...
  return (
    <>
      <motion.aside
        ...
        className={`fixed left-0 top-0 h-screen bg-[#1E2A44] text-white font-inter shadow-xl flex flex-col py-6 z-30 border-r border-[#2E7D7D]/30 m-0 p-0 ${className}`}
      >
        <div className={`flex flex-col ${isSidebarOpen ? 'items-center' : 'items-start'} gap-4 mb-8 px-4`}>
          <div className="flex items-center gap-4">
            <img
              ...
              alt="MutSyncHub Logo"
              className={`h-10 w-10 rounded-lg shadow-lg ${!isSidebarOpen && 'ml-1'}`}
            />
            {isSidebarOpen && (
              <span className="text-white font-sans font-extrabold text-lg tracking-wide">MutSyncHub</span>
            )}
          </div>
          {!isSidebarOpen && (
            <Button
        ...
        <nav className={`flex flex-col gap-4 w-full ${isSidebarOpen ? 'items-center' : 'items-start'} mt-2 flex-grow`}>
          {navLinks.map((link, index) => (
            <div key={link.to} className="relative group">
              ...
              >
                ...
                {isSidebarOpen && (
                  <span className="font-semibold text-base transition-all duration-200">{link.label}</span>
                )}
              </Link>
              {!isSidebarOpen && (
                <Tooltip id={`nav-${index}`} />
              )}
            </div>
          ))}
          ...
          <div className="mt-auto px-4 py-3">
            <div className="flex items-center justify-between w-full">
              ...
              >
                <Avatar>
                  <AvatarFallback className="bg-[#2E7D7D] text-white font-bold text-lg">
              ...
              {isSidebarOpen && (
                <div className="relative group">
                  <Button
                    ...
                    className="flex items-center gap-4 px-4 py-3 rounded-xl transition-all duration-200 font-sans text-gray-300 hover:bg-red-900/50 hover:text-red-400"
                    aria-label="Logout"
                    data-tooltip-id="logout"
                    data-tooltip-content="Logout"
                  >
                    <LogOut size={20} />
                    <span className="font-semibold text-base transition-all duration-200">Logout</span>
                  </Button>
                  <Tooltip id="logout" />
                </div>
              )}
              {!isSidebarOpen && (
                <div className="relative group">
                  <Button
                    onClick={handleLogout}
                    className="flex items-center gap-4 w-full px-4 py-3 rounded-xl transition-all duration-200 font-sans text-gray-300 hover:bg-red-900/50 hover:text-red-400"
                    aria-label="Logout"
                    data-tooltip-id="logout-minimized"
                    data-tooltip-content="Logout"
                  >
                    <LogOut size={20} />
                  </Button>
                  <Tooltip id="logout-minimized" />
                </div>
              )}
            </div>
            {isSidebarOpen && profileOpen && (
              ...
              >
                <div className="text-white font-semibold mb-2">{user?.displayName || 'User'}</div>
                <Button
