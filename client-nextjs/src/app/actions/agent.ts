...
export async function agentChat(message: string, threadId = "anon"): Promise<AgentChatResponse> {
  ...
  const requiresContact = history.filter((h) => h.role === "assistant").length >= 4; // after 4 turns
  await saveMessage(threadId, "user", message);
  await saveMessage(threadId, "assistant", content);
  return { role: "assistant", content, requiresContact };
}
...
export async function analyticsQuery(question: string, threadId = 'anon') {
  /* dummy example — replace with real /api/analytics call */
...
/* ---------- thin wrappers (Next.js auto-marks as Server Action) ---------- */
export async function chatWrapper(message: string, threadId?: string) { return agentChat(message, threadId); }
export async function reqWrapper(service: string, threadId?: string) { return gatherRequirements(service, threadId); }
export async function bookWrapper(date: string, time: string, email: string, threadId?: string) { return bookSlot(date, time, email, threadId); }
