...
const SignUpPage: React.FC = () => {
  ...
  const messages = [
    "Welcome to MutsynChub – unlock your analytics.",
    "Seamless signup. Powerful dashboards.",
  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    const { error } = await stack.signUp('email', { email, password });

    if (error) {
      setError(error.message);
    } else {
      router.push("/dashboard"); // adjust route to your app
    }
    setLoading(false);
  };
  ...
  const handleGoogleSignUp = async () => {
    ...
    const { error } = await stack.signUp('google');
