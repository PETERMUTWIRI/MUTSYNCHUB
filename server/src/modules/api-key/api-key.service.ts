...
export class ApiKeyService {
  ...
  async createApiKey(orgId: string, data: {
    ...
    try {
      ...
      this.logger.error(`Failed to create API key: orgId=${orgId}, error=${error instanceof Error ? error.message : JSON.stringify(error)}`, error instanceof Error ? error.stack : undefined);
      throw error;
    }
  }
  ...
  async validateApiKey(key: string, requestId?: string) {
    try {
      ...
      this.logger.error(
        `API key validation failed: requestId=${requestId}, error=${error instanceof Error ? error.message : JSON.stringify(error)}`,
  ...
  async revokeApiKey(id: string, orgId: string) {
    try {
      ...
      this.logger.error(`Failed to revoke API key: keyId=${id}, error=${error instanceof Error ? error.message : JSON.stringify(error)}`, error instanceof Error ? error.stack : undefined);
      throw error;
    }
  }
  ...
  async listApiKeys(orgId: string, includeRevoked = false) {
    try {
      ...
      this.logger.error(`Failed to list API keys: orgId=${orgId}, error=${error instanceof Error ? error.message : JSON.stringify(error)}`, error instanceof Error ? error.stack : undefined);
      throw error;
    }
  }
